...
class Network(object):
    ...
    def __init__(self, inputs, trainable=True):
        ...
        use_dropout_value = 1.0 if trainable else 0.0
        self.use_dropout = tf.placeholder_with_default(tf.constant(use_dropout_value),
                                                       shape=[],
                                                       name='use_dropout')
        #self.setup()
    ...
    def load(self, data_path, session, ignore_missing=False, skip_op=[]):
    ...
    @layer
    def pad(self, input, amount, name):
        return tf.pad(input, [[0, 0], [amount, amount], [amount, amount], [0, 0]], 'CONSTANT', name=name)
