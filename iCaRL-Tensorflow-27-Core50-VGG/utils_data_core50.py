...
def read_data(train_path, labels_list, files_from_cl):
    ...
    label = input_queue[1]
    image = tf.image.resize_images(tf.image.decode_png(image_file_content, channels=3), [image_size, image_size])
    return image, label

'''
    Reads files, unshuffled. File strings are used on the exemplar selection part (file names are kept in memory).
'''
...
'''
    Returns file names and labels for the training data
'''
