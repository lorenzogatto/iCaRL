...
'''
Gets aa variable or create one if it does not exist
'''
...
'''
From real values to probabilities
'''
...
'''
Batch normalization. During training it computes mean and variances that will be used in test time
'''
...
'''
ResNet network, from input to last layer (before softmax).
Return the scores, the values in the last layer
'''
...
def save_model(name, scope, sess):
    ...
    cPickle.dump(d, open(name, 'wb'), protocol=2)